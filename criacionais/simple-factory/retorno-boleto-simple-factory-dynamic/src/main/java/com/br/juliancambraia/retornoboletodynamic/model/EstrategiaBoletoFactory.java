 ...
 */
public abstract class EstrategiaBoletoFactory {
    ...
    static LeituraRetorno newStrategy(final String caminhoArquivo) {
        ...
        try {
            ...
            throw new UnsupportedOperationException("Classe " + nomeClasseEstrategia + " não existe para processar arquivo " + caminhoArquivo + " do " + nomeBanco);
    ...
    private static String getNomeBanco(final String caminhoArquivo) {
        ...
        if (!matcher.matches()) {
            throw new IllegalArgumentException("Nome do arquivo não está no formato Nome-Banco-NumeroArquivo.csv");
        }
